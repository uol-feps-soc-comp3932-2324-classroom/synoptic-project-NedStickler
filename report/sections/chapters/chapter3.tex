...
This chapter overviews and justifies our implementation, provides performance results, and presents our findings.
...
A larger batch size produces a smoother gradient and therefore more stable training, however the hardware available restricts how large the batch size can be~\cite{batchSizeTest}. Employing a method of trial and error identifies a batch size suitable for training the SR reconstruction model whilst adhering to memory constraints. A sample dataset of size 450 is used to test different batch sizes, with the intention of later increasing the dataset size to increase training examples. Firstly, training of the SRGAN model begins with a batch size and a single image patch per training example. The system attempts to allocate the required memory resources and computing power from the GPU but is unable to, resulting in an OOM exception. The batch size is then reduced, and the process is repeated until no OOM exception occurs. In practice the first batch size selected was $b = 30$, which resulted in an OOM exception. This value was reduced until a final batch size of $b = 15$ was selected. The next step is to test the memory constraints on the number of patches taken from each training example. The number of patches was reduced until model training no longer yielded an OOM exception. The initial value was $p = 16$, as suggested by Ledig \etal\, with the test yielding $p = 10$ patches per training instance. Further testing showed that for $p = 10$ and $p = 9$ that training would begin, but then halt at seemingly random points in the process. To combat this, the number of patches was further reduced to $p = 8$, which yielded no OOM exceptions. Finally, the dataset size is selected by employing the same trial and error approach. We yielded dataset size of $n = 945$. The final sizes are a batch size of $b = 15$, number of patches $p = 10$ and dataset size $n = 945$.

We can also calculate the number of epochs and the number of steps per epoch (update iterations). An epoch step is defined as model training for one batch. Our total number of steps per epoch is:
...
\begin{table}
    ...
    \begin{tabular}{ccccccc}
        ...
        \bottomrule
    \end{tabular}
    \caption{SSIM and PSNR results calculated on the test subset of NWPU-RESISC45, Set5, and Set14.}\label{fig:performance_results}
\end{table}

Calculating our performance metrics for each of the trained SRGAN models enables us to refer back to the conjecture presented in section~\ref{subsec:improving_loss}: `using the feature maps from more accurate image classification models in the perceptual loss component of SRGAN-based models will increase SR reconstruction performance'. We have the accuracy metrics for each of the pretrained classifiers, and we now have the SR reconstruction capabilities of each of the models, measured with SSIM and PSNR.\@ Therefore, we can review how these performance metrics change with classifier accuracy. Plotting the Top-1 accuracy against SSIM and PSNR achieves this goal, with a line of best fit acting as a trend line. Figure~\ref{fig:performance_metric_vs_classifier_accuracy} shows the outcome of this simple test.
\begin{figure}
    \centering
    ...
    \end{minipage}
    \caption{Two scatter plots showing how the performance metrics of SRGAN models changed based on the accuracy of the classifier used for the perceptual loss component.}\label{fig:performance_metric_vs_classifier_accuracy}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Addressing our conjecture:} Comparing classifier accuracy and reconstruction capabilities in figure~\ref{fig:performance_metric_vs_classifier_accuracy} suggests that there is a positive relationship between classification accuracy and SR reconstruction capabilities.
